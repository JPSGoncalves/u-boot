 ...
 * Ported by: Joao Paulo Goncalves <joao.goncalves@toradex.com> 
 */
...
static u16 cdns_salvo_read(struct cdns_salvo_phy *phy, u32 offset, u32 reg)
{
	return (u16)readl(phy->base + offset +
		reg * (1 << phy->data->reg_offset_shift));
}

static void cdns_salvo_write(struct cdns_salvo_phy *phy, u32 offset,
			     u32 reg, u16 val)
{
	writel(val, phy->base + offset +
		reg * (1 << phy->data->reg_offset_shift));
}

static int cnds_salvo_phy_init(struct phy *phy)
{
	struct cdns_salvo_phy *priv = dev_get_priv(phy->dev);
	struct cdns_salvo_data *data = priv->data;
	...
	for (int i = 0; i < data->init_sequence_length; i++) {
		const struct cdns_reg_pairs *reg_pair = data->init_sequence_val + i;
		cdns_salvo_write(priv, USB3_PHY_OFFSET, reg_pair->off, reg_pair->val);
	}

	/* RXDET_IN_P3_32KHZ, Receiver detect slow clock enable */
	value = cdns_salvo_read(priv, USB3_PHY_OFFSET, TB_ADDR_TX_RCVDETSC_CTRL);
	value |= RXDET_IN_P3_32KHZ;
	cdns_salvo_write(priv, USB3_PHY_OFFSET, TB_ADDR_TX_RCVDETSC_CTRL,
			 RXDET_IN_P3_32KHZ);

	value = cdns_salvo_read(priv, USB2_PHY_OFFSET, UTMI_REG15);
	value &= ~TXVALID_GATE_THRESHOLD_HS_MASK;
	cdns_salvo_write(priv, USB2_PHY_OFFSET, UTMI_REG15,
			 value | TXVALID_GATE_THRESHOLD_HS_0US);

	cdns_salvo_write(priv, USB2_PHY_OFFSET, UTMI_AFE_RX_REG5, 0x5);

	value = cdns_salvo_read(priv, USB2_PHY_OFFSET, UTMI_AFE_RX_REG0);
	value &= ~RX_USB2_DISCONN_MASK;
	value = FIELD_PREP(RX_USB2_DISCONN_MASK, priv->usb2_disconn);
	cdns_salvo_write(priv, USB2_PHY_OFFSET, UTMI_AFE_RX_REG0, value);
...
static int cdns_salvo_phy_power_on(struct phy *phy)
{
	...
	return clk_enable(priv->phy_clk);
}
...
static int cdns_salvo_phy_power_off(struct phy *phy)
{
	...
	return clk_disable(priv->phy_clk);
}

static struct phy_ops cnds_salvo_phy_ops = {
	.init = cnds_salvo_phy_init,
...
static const struct udevice_id cdns_salvo_phy_ids[] = {
	{ .compatible = "nxp,salvo-phy", .data = NXP_DRIVER_DATA},
	{ }
};

static struct cdns_salvo_data *cdns_salvo_get_driver_data(ulong data)
{
	switch(data)
	{
		case NXP_DRIVER_DATA:
			return &cdns_nxp_salvo_data;			
		default:
			return NULL;
	}
}

static int cnds_salvo_phy_probe(struct udevice *dev)
{
	struct cdns_salvo_phy *priv = dev_get_priv(dev);

	
	priv->data = cdns_salvo_get_driver_data(dev_get_driver_data(dev));

	if (!priv->data)
	{
		dev_err(dev, "Failed to get driver data: invalid value %lu", 
			dev_get_driver_data(dev));
		return -EINVAL;	
	}
	
	if (CONFIG_IS_ENABLED(CLK)) {
		priv->phy_clk = devm_clk_get(dev, "usb3_phy_clk");
		if (IS_ERR(priv->phy_clk)) {
			dev_err(dev, "Failed to get phy clk: %ld\n", PTR_ERR(priv->phy_clk));
...
U_BOOT_DRIVER(cnds_salvo_phy) = {
	.name	= "cdns,salvo",
	.id	= UCLASS_PHY,
	.of_match = cdns_salvo_phy_ids,
	.ops = &cnds_salvo_phy_ops,
	.probe = cnds_salvo_phy_probe,
	.priv_auto	= sizeof(struct cdns_salvo_phy),
};
